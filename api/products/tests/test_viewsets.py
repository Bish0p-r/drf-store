...
class TestProductSizeViewSet:
    ...
    def test_remove_from_wishlist(self, client, user, product_factory):
        ...
        response = client.post(
            f"{self.endpoint}{product1.public_id}/remove_from_wishlist/"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_size_retrieve(self, client, user, product_factory):
        ...
        response = client.get(
            f"{self.endpoint}{product.public_id}/size/{size.public_id}/"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_add_or_remove_cart(self, client, user, product_factory):
        ...
        size = product.sizes.first()
        response = client.post(
            f"{self.endpoint}{product.public_id}/size/{size.public_id}/add_to_cart/",
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        client.force_authenticate(user=user)
        response = client.post(
        ...
        response = client.post(
            f"{self.endpoint}{product.public_id}/size/{size.public_id}/remove_from_cart/",
            data={"quantity": 2},
        )
        assert response.status_code == status.HTTP_200_OK
